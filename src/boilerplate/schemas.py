...
class Persons(BaseModel):
    ...
    def __call__(self):
        ...
        self.persons = {
            persons.index[k]: v for k, v in enumerate(adapter.validate_python(records))
        }
    ...
    def engineer(self) -> pd.DataFrame:
        ...
        self.data = pd.DataFrame.from_dict(
            {k: v.model_dump() for k, v in self.persons.items()}, orient="index"
        )
    ...
    def explore(self, data: pd.DataFrame = None):
        ...
        util.project_dataframe_in_tensorboard(data, "embedding", self.config.output)
    ...
    def save(self, directory: Path | list = None):
        ...
        # Save the `persons` as a dill.
        path = directory / Path(*["persons.pkl"])
        with open(path, "wb") as f:
            dill.dump(self, f, protocol=dill.HIGHEST_PROTOCOL)

        # Save the `persons` as a JSON.
    ...
    @staticmethod
    def load(
        directory: Path | list = Path(__file__).resolve().parent.parents[1]
        / Path(*["output"]),
    ) -> dict:
        if isinstance(directory, list):
            directory = Path(*directory)

        # Load the `persons` from the dill.
        path = directory / Path(*["persons.pkl"])
        if not path.exists():
            raise FileNotFoundError(f"No saved persons file found at {path}")
        with open(path, "rb") as f:
            persons_pkl = dill.load(f)

        # Load the `persons` from the JSON.
        path = directory / Path(*["persons.json"])
        if not path.exists():
            raise FileNotFoundError(f"No saved persons file found at {path}")
        persons_json = path.read_bytes()
        persons_json = TypeAdapter(Dict[str, Person]).validate_json(persons_json)
